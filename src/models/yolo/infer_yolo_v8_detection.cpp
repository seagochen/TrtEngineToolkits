...
InferYoloV8Obj::InferYoloV8Obj(
    const std::string& engine_path, int maximum_batch, int maximum_items): InferModelBaseMulti(
        engine_path,
    ...
    this->maximum_batch = maximum_batch;
...
std::vector<Yolo> InferYoloV8Obj::postprocess(const int batchIdx, const float cls, const float iou) {
    ...
    int results = sct_yolo_post_proc(cuda_device_ptr, g_vec_output, infer_features, infer_samples, cls);
    if (results < 0) {
    ...
    // 4) 将结果转换为 Yolo 对象
    std::vector<Yolo> yolo_results;
    host_xywh_to_xyxy_yolo(g_vec_output, yolo_results, infer_features, results);
