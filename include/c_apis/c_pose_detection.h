 ...
 *
 * @param out_results 指向 C_Extended_Person_Feats* 数组的指针的指针。
 * 函数会内部为此数组分配内存，调用者需要调用 free_pose_detection_results 释放。
 * @param out_num_detected 检测到的物体数量的指针。
...
/**
 * @brief 释放由 run_pose_detection_pipeline 分配的内存。
 *
 * @param results 指向 C_Extended_Person_Feats 数组的指针。
 */
